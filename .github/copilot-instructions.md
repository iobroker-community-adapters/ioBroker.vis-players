...
8. [Widget Development Best Practices](#widget-development-best-practices)
9. [Widget-Specific Development Guidelines](#widget-specific-development-guidelines)
...
- **console.log**: Replace with `adapter.log.debug()` or remove. **Exception:** For widget adapters like vis-players, `console.log` may be appropriate for debugging widget initialization and major state changes
...
```json
{
  "scripts": {
    ...
    // lint runs first; if it fails, validate:translations is skipped (lint-first principle)
  }
}
```
